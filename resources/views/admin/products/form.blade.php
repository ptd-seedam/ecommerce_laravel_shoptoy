                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">
                                    {{ isset($product) ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm' }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <form class="form-horizontal" method="POST" enctype="multipart/form-data"
                            action="{{ isset($product) ? route('admin.update_product', $product->id) : route('admin.store_product') }}">
                            @csrf
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    <label for="name" class="col-sm-3 text-end control-label col-form-label">Tên sản
                                        phẩm</label>
                                    <div class="col-sm-9">
                                ...
                                <div class="form-group row">
                                    <label for="stock" class="col-sm-3 text-end control-label col-form-label">Số
                                        lượng</label>
                                    <div class="col-sm-9">
                                ...
                                <div class="form-group row">
                                    <label for="category_id" class="col-sm-3 text-end control-label col-form-label">Danh
                                        mục</label>
                                    <div class="col-sm-9">
                                        <select class="form-select" id="category_id" name="category_id">
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('category_id', $product->CategoryId ?? '') == $category->id ? 'selected' : '' }}>
                                                    {{ $category->Name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    <label for="images" class="col-sm-3 text-end control-label col-form-label">Hình
                                        ảnh</label>
                                    <div class="col-sm-9">
                                        @if (isset($images) && count($images) > 0)
                                            @foreach ($images as $index => $image)
                                                <div class="input-group mb-3">
                                                    <input type="file" name="existing_images[]" class="form-control"
                                                        placeholder="Tải ảnh lên để thay đổi">
